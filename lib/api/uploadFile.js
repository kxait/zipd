    ...
    getToken(token)
        .then(user => {
            ...
            for(i in files) {
                ...
                var size = 0;
                try {
                    var fname = file.originalname.match(/(?:\.([^.]+))?$/)[0];
                    var type = file.originalname.match(/(?:\.([^.]+))?$/)[1];
            ...
            res.send({status: "success", message: sizeSum + "KB of files received, will sync soon, may not appear in list instantly"});
            ...
            for(var i in files) {
                ...
                (currentFile => {
                    new Promise((resolve, reject) => {
                        ...
                        var fname = currentFile.originalname.substr(0, currentFile.originalname.lastIndexOf('.'));
                        var type = currentFile.originalname.substr(
                            currentFile.originalname.lastIndexOf('.') + 1);
