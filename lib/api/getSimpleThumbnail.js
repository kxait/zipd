...
createThumbnailStreamFromStream = (input, size, id, seek = "00:00:02.00") => new Promise((resolve, reject) => {
    ...
    result.then(() => {resolve()});
    result.catch(e => {
        console.error("error in createThumbnailStreamFromStream: " + e + ", trying again without seek"); 
        createThumbnailStreamFromStream(input, size, id, "00:00:00.00")
            .then(() => resolve())
            .catch(e => {
                console.error("could not create thumbnail after two tries: " + e)
                reject(e);
            })
    });
});
...
respondWithImageThumbnail = (_attach) => (req, res) => {
    function pipeImageStream(id) {
        ...
        res.type(".jpeg");
        fs.createReadStream(getThumbsPath(id)).pipe(res);
    }
